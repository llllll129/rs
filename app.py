    ...
    if active == 0:
        ...
        if not structure: st.info("📭 老师暂未搭建章节结构")
            ...
            for si, sec in enumerate(ch_data.get("sections", [])):
                for ki, kp in enumerate(sec.get("knowledge_points", [])):
                    all_kps.append({
                        ...
                        "textbook_images": kp.get("textbook_images", []) or [kp.get("textbook_image", "")] if kp.get("textbook_image") else kp.get("textbook_images", []),
            ...
            if not all_kps: st.info("本章暂无知识点")
                ...
                with left_col:
                    if ch_data.get("framework_image"): st.image(ch_data["framework_image"], use_container_width=True)
                    else: st.info("📊 暂无框架图")
                with right_col:
                            ...
                            with st.expander("##### 🌰 生活实例"):
                                for ex in kp["selected_examples"]:
                                    if isinstance(ex, dict):
                                        if ex.get("type") == "image" and os.path.exists(ex.get("content", "")): st.image(ex["content"])
                                        elif ex.get("type") == "video" and os.path.exists(ex.get("content", "")): st.video(ex["content"])
        ...
        for i, sec in enumerate(sections):
            ...
            if f"new_kp_title_val_{i}" in st.session_state:
                ...
                if st.button(f"✅ 确认添加知识点", key=f"new_kp_confirm_{i}", type="primary"):
                    ...
                    for k in [f"new_kp_title_val_{i}",f"new_kp_textbook_{i}",f"new_think_{i}",f"new_kp_img_list_{i}",f"new_kp_tb_img_list_{i}"]:
                                ...
                                with cols[idx % 4]:
                                    if os.path.exists(img_path):
                                        ...
                                        if st.button("🗑️", key=f"del_edit_tb_img_{i}_{j}_{idx}"):
                                            tb_images.pop(idx); kp['textbook_images'] = tb_images
                                ...
                                with cols[idx % 4]:
                                    if os.path.exists(e['content']):
                                        ...
                                        if st.button("🗑️", key=f"del_edit_life_img_{i}_{j}_{idx}"):
                                            current_examples.remove(e); save_json(structure, CHAPTER_STRUCTURE_FILE); st.rerun()
        ...
        if st.button("🎲 自动生成问题", key="class_qa_gen"):
            with st.spinner("生成中..."):
                ...
                if qa_question_type == "选择题":
                    sp = f"你是遥感课程教师。请围绕「{qa_knowledge if qa_knowledge else qa_chapter}」出1道单选题。输出格式：\n【题目】xxx\nA. xxx  B. xxx  C. xxx  D. xxx\n【答案】X\n【解析】xxx"
                elif qa_question_type == "填空题":
                    sp = f"你是遥感课程教师。请围绕「{qa_knowledge if qa_knowledge else qa_chapter}」出1道填空题，用______标记空白处。输出格式：\n【题目】xxx\n【答案】xxx"
                else:
                    sp = f"你是遥感课程教师。请围绕「{qa_knowledge if qa_knowledge else qa_chapter}」出1道简答题。输出格式：\n【题目】xxx\n【答案要点】xxx"
