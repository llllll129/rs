...
@st.cache_resource
def init():
    model = SentenceTransformer("models/text2vec-base-chinese", local_files_only=True)
...
def load_json(path):
    if not os.path.exists(path):
        if path in [STUDY_LOG_FILE, CHAPTER_STRUCTURE_FILE]:
            return {}
        return []
...
要求：
1. 不要简单重复概念，要引导学生深入理解原理
2. 可以结合实际应用场景提问
3. 可以让学生推导或解释某个现象的本质原因
...
def auto_grade_quiz(question, correct_answer, student_answer, q_type):
    if q_type == "选择题":
        ...
        if student_answer.strip() == correct_answer.strip():
            ...
            return (60, f"部分正确，参考答案：{correct_answer}")
    else:
        ...
        for line in result.split("\n"):
            if "【得分】" in line:
                try:
                    score = int(line.split("【得分】")[1].split("分")[0].strip())
                except:
                    pass
        return score, result
...
def log_interaction(log_type, data):
    ...
    data["log_type"] = log_type  # 改成 log_type，不覆盖原有的 type
...
def analyze_logs_by_type(log_type):
    ...
    with open(LOG_FILE, "r", encoding="utf-8") as f:
        for line in f:
            if line.strip():
                r = json.loads(line)
                if r.get("log_type") == log_type:  # 改成 log_type
                    records.append(r)
    return records
...
if role == "👨‍🎓 学生端":
    ...
    if active == 0:
        ...
        if not structure:
            ...
            for si, sec in enumerate(ch_data.get("sections", [])):
                for ki, kp in enumerate(sec.get("knowledge_points", [])):
                    all_kps.append({
                        ...
                        "textbook_images": kp.get("textbook_images", [kp.get("textbook_image", "")]) if not kp.get("textbook_images") else kp.get("textbook_images", []),
            ...
            if not all_kps:
                ...
                with left_col:
                    if ch_data.get("framework_image"):
                        st.image(ch_data["framework_image"], use_container_width=True)
                    else:
                        st.info("📊 暂无框架图")
                with right_col:
                    if learned_count >= total:
                        st.success("🎉 本章所有知识点已学习完毕！")

                    if current_idx >= total:
                        st.info("已是最后一个知识点，可以回顾前面的内容或参加章节检测。")
                    else:
                            ...
                            with st.expander("##### 🌰 生活实例"):
                                for ex in kp["selected_examples"]:
                                    if isinstance(ex, dict):
                                        if ex.get("type") == "image" and os.path.exists(ex.get("content", "")):
                                            st.image(ex["content"])
                                        elif ex.get("type") == "video" and os.path.exists(ex.get("content", "")):
                                            st.video(ex["content"])
                                        else:
                                            if ex.get("content"): st.info(ex["content"])
                                    elif ex: st.info(ex)
                        if kp.get("think_question"):
                            ...
                            if st.button("📤 提交思考题", key=f"submit_think_{sel_ch}_{current_idx}") and think_answer:
                                with st.spinner("评阅中..."):
                                    ev = evaluate_think_answer(kp['think_question'], think_answer, sel_ch)
                        ...
                        with col2:
                            if current_idx in learned:
                                ...
                                if think_q:
                                    submitted = any(
                                        r.get('chapter') == sel_ch and r.get('kp_title') == kp['kp_title']
                                        for r in analyze_logs_by_type("think")
                                        if f"{r.get('student_id', '')}_{r.get('student_name', '')}" == student_key
                                    )
                                    if not submitted:
                                        st.warning("⚠️ 请先提交该知识点的思考题，再标记已学")
                                        st.stop()
                                learned.add(current_idx)
                                my_log[sel_ch]["learned"] = list(learned)
                                if current_idx < total - 1:
                                    my_log[sel_ch]["current_idx"] = current_idx + 1
                                save_json(study_log, STUDY_LOG_FILE)
                                st.rerun()
                        with col3:
            ...
            if not isinstance(class_qa, list) or not class_qa:
                st.info("📭 老师暂未发布课堂问答")
            else:
                    ...
                    for idx, item in enumerate(pre_qa):
                        with st.expander(f"问题{idx + 1}", expanded=(idx == 0)):
                            ...
                            if st.session_state[submit_key]:
                                ...
                                if qtype == "选择题":
                                    stu_qa_answer = st.radio("你的答案：", ["A", "B", "C", "D"],
                                                             key=f"pre_qa_{sel_qa_ch}_{idx}")
                                else:
                                    stu_qa_answer = st.text_area("✍️ 你的回答：", height=100,
                                                                 key=f"pre_qa_{sel_qa_ch}_{idx}")
                                ...
                                if st.button(f"📤 提交", key=f"submit_pre_qa_{sel_qa_ch}_{idx}") and stu_qa_answer:
                                    ...
                                    if qtype == "选择题":
                                        ...
                                        s, fb = auto_grade_quiz(item['question'], item.get('reference', ''),
                                                                stu_qa_answer, qtype)
                                        ev = fb;
                                        reviewed = False
                                        st.success("✅ 已提交，老师可二次评阅")
                                    log_interaction("class_qa", {
                                        "student_id": student_id, "student_name": student_name,
                                        "chapter": sel_qa_ch, "type": item['type'], "q_type": qtype,
                                        "question": item['question'], "student_answer": stu_qa_answer,
                                        "score": s, "evaluation": ev, "reviewed": reviewed
                                    })
                                    st.rerun()
        ...
        if not isinstance(quizzes, list) or not quizzes:
            ...
            my_log = study_log.get("students", {}).get(student_key, {})
            ...
            for q in quizzes:
                ...
                names.append(f"{ch}（{q_type_info}，{len(q['questions'])}题）{'✅' if can else '🔒'}")

                available.append((q, can))
                ...
                for i, qi in enumerate(quiz["questions"]):
                    ...
                    if st.session_state[submit_key]:
                        ...
                        if q_type == "选择题":
                            ...
                            stu = st.radio("答案：", ["A", "B", "C", "D"], key=f"qz_{i}")

                        elif q_type == "填空题":

                            stu = st.text_area("填空答案：", height=68, key=f"qz_{i}")

                        else:

                            stu = st.text_area("回答：", height=80, key=f"qz_{i}")
                        ...
                        if st.button(f"提交第{i + 1}题", key=f"qsub_{i}"):
                            ...
                            if q_type == "选择题":
                                ...
                                option_lines = [l for l in question_text.split("\n") if
                                                ...
                                                l.strip().startswith(("A.", "B.", "C.", "D."))]

                                options = {}

                                for ol in option_lines:

                                    if len(ol) >= 2 and ol[1] == ".":
                                        letter = ol[0]
                                ...
                                if selected_content and correct_content:

                                    if selected_content == correct_content or selected_content[:10] == correct_content[
                                                                                                       :10]:
                            ...
                            log_interaction("quiz", {"student_id": student_id, "student_name": student_name,

                                                     "chapter": quiz['chapter'], "question": qi['question'],

                                                     "q_type": q_type, "correct_answer": qi['answer'],

                                                     "student_answer": stu, "score": s, "feedback": fb,
                ...
                if p["chapter"] == sel:
                    ...
                    if p.get("is_file"):
                                ...
                                with col_view:
                                    ...
                                    if p['title'].lower().endswith('.pdf'):
                                        ...
                                        with open(p['file_path'], "rb") as f:
                                            ...
                                            st.download_button("📥 下载论文", f.read(), file_name=p['title'],
                                                               key=f"dl_paper_{p.get('file_path', '')}")
                                        ...
                                        st.text_area("论文内容：", content, height=300)
                                        ...
                                        try:
                                            ...
                                            doc = Document(p['file_path'])

                                            content = "\n".join([para.text for para in doc.paragraphs])

                                            st.text_area("论文内容：", content[:5000], height=300)
                        ...
                        with st.expander(f"📄 {p['title']}"):
                            ...
                            st.write(f"**作者**：{p.get('author', '')}")

                            st.write(f"**摘要**：{p.get('abstract', '')}")
...
# ==================== 教师端 ====================
    ...
    with tabs[0]:
        ...
        if sel_ch == "➕ 新建章节":
            ...
            if st.button("✅ 创建章节", key="create_ch_btn") and new_ch:
                ...
                st.success(f"✅ 章节「{new_ch}」已创建")
                st.rerun()
            st.stop()
        ch_data = structure[sel_ch]

        st.markdown("### 📊 本章框架图")
        if ch_data.get("framework_image"): st.image(ch_data["framework_image"], width=500)
        uploaded_img = st.file_uploader("上传框架图（图片或Word）", type=["png","jpg","jpeg","docx"], key="fw_upload")
        if uploaded_img:
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            img_path = os.path.join(UPLOAD_DIR, f"fw_{ts}_{uploaded_img.name}")
            with open(img_path, "wb") as f: f.write(uploaded_img.getbuffer())
            ch_data["framework_image"] = img_path
            save_json(structure, CHAPTER_STRUCTURE_FILE)
            st.success("✅ 框架图已上传")
            st.rerun()
        ...
        if st.button("➕ 添加小节", key="add_section_btn") and new_sec_title:
            ...
            save_json(structure, CHAPTER_STRUCTURE_FILE)
            st.success(f"✅ 小节「{new_sec_title}」已添加")
            st.rerun()

        for i, sec in enumerate(sections):
            st.markdown("---")
            st.markdown(f"### 📖 {sec['title']}（{len(sec.get('knowledge_points', []))}个知识点）")
            ...
            if st.button(f"🔍 检索课本内容", key=f"new_kp_search_{i}") and kp_title_input:
                with st.spinner("正在检索并整理课本内容..."):
                    textbook = search_and_format_textbook(kp_title_input, chapter_filter=sel_ch, n_results=4)
                st.session_state[f"new_kp_textbook_{i}"] = textbook
                st.session_state[f"new_kp_title_val_{i}"] = kp_title_input
                st.session_state[f"new_think_{i}"] = ""
                st.session_state[f"new_kp_img_list_{i}"] = []
                st.session_state[f"new_kp_tb_img_list_{i}"] = []
                st.rerun()

            if f"new_kp_title_val_{i}" in st.session_state:
                kp_title_val = st.session_state[f"new_kp_title_val_{i}"]
                textbook = st.session_state.get(f"new_kp_textbook_{i}", "")
                ...
                if f"new_kp_tb_img_list_{i}" not in st.session_state:
                    st.session_state[f"new_kp_tb_img_list_{i}"] = []
                tb_multi = st.file_uploader("选择课本配图（可多选）", type=["png","jpg","jpeg"],
                                            accept_multiple_files=True, key=f"tb_multi_{i}")
                if tb_multi and not st.session_state.get(f"tb_done_{i}", False):
                    st.session_state[f"tb_done_{i}"] = True
                    for uf in tb_multi:
                        ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
                        ip = os.path.join(UPLOAD_DIR, f"tbimg_{ts}_{uf.name}")
                        with open(ip, "wb") as f: f.write(uf.getbuffer())
                        st.session_state[f"new_kp_tb_img_list_{i}"].append(ip)
                    st.rerun()
                        ...
                        with cols[idx % 4]:
                            if os.path.exists(p):
                                try:
                                    ...
                                    st.caption("图片已损坏")
                                if st.button("🗑️", key=f"dtb_{i}_{idx}"):
                                    st.session_state[f"new_kp_tb_img_list_{i}"].pop(idx)
                                    st.rerun()
                ...
                if st.button("🎲 自动生成思考题", key=f"gen_think_btn_{i}", use_container_width=True):
                    with st.spinner("生成中..."):
                        st.session_state[f"new_think_{i}"] = generate_think_question(kp_title_val, edited_textbook, sel_ch)
                    st.rerun()
                ...
                if f"new_kp_img_list_{i}" not in st.session_state:
                    st.session_state[f"new_kp_img_list_{i}"] = []
                img_multi = st.file_uploader("选择图片（可多选）", type=["png","jpg","jpeg"],
                                             accept_multiple_files=True, key=f"img_multi_{i}")
                if img_multi and not st.session_state.get(f"img_done_{i}", False):
                    st.session_state[f"img_done_{i}"] = True
                    for uf in img_multi:
                        ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
                        ip = os.path.join(UPLOAD_DIR, f"ex_{ts}_{uf.name}")
                        with open(ip, "wb") as f: f.write(uf.getbuffer())
                        st.session_state[f"new_kp_img_list_{i}"].append(ip)
                    st.rerun()
                ...
                if img_list:
                    ...
                    for idx, p in enumerate(img_list):
                        with cols[idx % 4]:
                            if os.path.exists(p):
                                st.image(p, width=100)
                                if st.button("🗑️", key=f"di_{i}_{idx}"):
                                    st.session_state[f"new_kp_img_list_{i}"].pop(idx); st.rerun()

                col_vid = st.columns([1])[0]
                with col_vid:
                    uploaded_video_ex = st.file_uploader("🎬 视频（可选）：", type=["mp4", "avi", "mov"], key=f"new_kp_vid_{i}")
                video_path_ex = ""
                if uploaded_video_ex is not None:
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    video_path_ex = os.path.join(UPLOAD_DIR, f"ex_v_{ts}_{uploaded_video_ex.name}")
                    with open(video_path_ex, "wb") as f: f.write(uploaded_video_ex.getbuffer())
                    st.success("✅ 视频已上传")
                ...
                if st.button(f"✅ 确认添加知识点", key=f"new_kp_confirm_{i}", type="primary"):
                    ...
                    for ip in st.session_state.get(f"new_kp_img_list_{i}", []):
                        examples.append({"type": "image", "content": ip})
                    ...
                    kps.append({"title": kp_title_val, "textbook_content": edited_textbook, "textbook_images": tb_imgs,
                                "selected_examples": examples, "think_question": final_think})
                    save_json(structure, CHAPTER_STRUCTURE_FILE)
                    for k in [f"new_kp_title_val_{i}", f"new_kp_textbook_{i}", f"new_think_{i}",
                              f"new_kp_img_list_{i}", f"new_kp_tb_img_list_{i}"]:
                        st.session_state.pop(k, None)
                    st.success(f"✅ 知识点「{kp_title_val}」已添加！")
                    st.rerun()
                ...
                for j, kp in enumerate(kps):
                    with st.expander(f"📌 {j+1}. {kp['title']}", expanded=False):
                        ...
                        if st.button(f"🔍 重新检索课本", key=f"re_search_{i}_{j}"):
                            with st.spinner("检索中..."):
                                result = search_and_format_textbook(edit_title or kp['title'], chapter_filter=sel_ch, n_results=4)
                            st.session_state[f"re_result_{i}_{j}"] = result
                            st.rerun()
                        if f"re_result_{i}_{j}" in st.session_state:
                            st.markdown("📖 **新检索结果：**")
                            st.text(st.session_state[f"re_result_{i}_{j}"])
                        ...
                        st.markdown("##### 🖼️ 课本配图")
                        tb_images = kp.get('textbook_images', [])
                        if not tb_images and kp.get('textbook_image'):
                            tb_images = [kp['textbook_image']]
                        if tb_images:
                            ...
                            for idx, img_path in enumerate(tb_images):
                                with cols[idx % 4]:
                                    if os.path.exists(img_path):
                                        ...
                                        if st.button("🗑️", key=f"del_edit_tb_img_{i}_{j}_{idx}"):
                                            tb_images.pop(idx)
                                            kp['textbook_images'] = tb_images
                                            save_json(structure, CHAPTER_STRUCTURE_FILE)
                                            st.rerun()
                        edit_tb_multi = st.file_uploader("补充课本配图（可多选）", type=["png","jpg","jpeg"],
                                                          accept_multiple_files=True, key=f"edit_tb_multi_{i}_{j}")
                        ...
                        if st.button("🎲 重新生成思考题", key=f"re_gen_btn_{i}_{j}", use_container_width=True):
                            with st.spinner("生成中..."):
                                st.session_state[f"edit_think_{i}_{j}"] = generate_think_question(edit_title or kp['title'], edit_textbook, sel_ch)
                            st.rerun()
                            ...
                            for idx, e in enumerate(life_images):
                                with cols[idx % 4]:
                                    if os.path.exists(e['content']):
                                        st.image(e['content'], width=100)
                                        if st.button("🗑️", key=f"del_edit_life_img_{i}_{j}_{idx}"):
                                            current_examples.remove(e)
                                            save_json(structure, CHAPTER_STRUCTURE_FILE)
                                            st.rerun()

                        for e in current_examples:
                            if isinstance(e, dict):
                                if e.get('type') == 'video' and e.get('content') and os.path.exists(e['content']):
                                    st.video(e['content'])
                        col_eimg, col_evid = st.columns([1, 1])
                        with col_eimg:
                            edit_img_multi = st.file_uploader("补充图片（可多选）", type=["png","jpg","jpeg"],
                                                               accept_multiple_files=True, key=f"edit_img_multi_{i}_{j}")
                            if edit_img_multi and not st.session_state.get(f"edit_img_done_{i}_{j}", False):
                                ...
                                for uf in edit_img_multi:
                                    ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
                                    ip = os.path.join(UPLOAD_DIR, f"edit_{ts}_{uf.name}")
                                    with open(ip, "wb") as f: f.write(uf.getbuffer())
                                    current_examples.append({"type":"image","content":ip})
                                save_json(structure, CHAPTER_STRUCTURE_FILE)
                                st.rerun()
                        with col_evid:
                            ...
                            if edit_vid is not None:
                                ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                                vp = os.path.join(UPLOAD_DIR, f"edit_{ts}_{edit_vid.name}")
                                with open(vp, "wb") as f: f.write(edit_vid.getbuffer())
                                current_examples.append({"type": "video", "content": vp})
                                save_json(structure, CHAPTER_STRUCTURE_FILE)
                                st.success("✅ 视频已添加")
                        ...
                        with col_save:
                            if st.button("💾 保存修改", key=f"save_kp_{i}_{j}"):
                                ...
                                for e in current_examples:
                                    if isinstance(e, dict) and e.get('type') in ('image', 'video') and e.get('content'):
                                        examples.append(e)
                                final_think = st.session_state.get(f"edit_think_{i}_{j}", edit_think_input)
                                kp.update({"title": edit_title, "textbook_content": st.session_state.get(f"edit_tb_{i}_{j}", edit_textbook),
                                           "textbook_images": tb_images, "selected_examples": examples, "think_question": final_think})
                                save_json(structure, CHAPTER_STRUCTURE_FILE)
                                st.success("✅ 已保存")
                                st.rerun()
                        with col_del:
                            if st.button("🗑️ 删除", key=f"del_kp_{i}_{j}"):
                                kps.pop(j)
                                save_json(structure, CHAPTER_STRUCTURE_FILE)
                                st.success("🗑️ 已删除")
                                st.rerun()
    ...
    with tabs[1]:
        ...
        qa_knowledge = st.text_input("关联知识点（用于AI出题）：", key="class_qa_knowledge",
                                     placeholder="例如：普朗克辐射定律")
        qa_question = st.text_area("输入问题：", key="class_qa_input", placeholder="输入问题，或点击下方按钮自动生成")

        if st.button("🎲 自动生成问题", key="class_qa_gen"):
            with st.spinner("生成中..."):
                if qa_knowledge:
                    ctx = "\n".join(search_textbook(qa_knowledge, chapter_filter=qa_chapter, n_results=5))
                else:
                    ctx = "\n".join(search_textbook(f"{qa_chapter} 核心知识点", chapter_filter=qa_chapter, n_results=5))
                if qa_question_type == "选择题":
                    sp = f"你是遥感课程教师。请围绕「{qa_knowledge if qa_knowledge else qa_chapter}」出1道单选题。用于{qa_type}。"
                    if qa_type == "课中随堂检测":
                        sp += "要求：难度适中偏难，考察理解和应用，选项要有干扰性。输出格式：\n【题目】xxx\nA. xxx  B. xxx  C. xxx  D. xxx\n【答案】X\n【解析】xxx"
                    else:
                        sp += "输出格式：\n【题目】xxx\nA. xxx  B. xxx  C. xxx  D. xxx\n【答案】X\n【解析】xxx"
                elif qa_question_type == "填空题":
                    sp = f"你是遥感课程教师。请围绕「{qa_knowledge if qa_knowledge else qa_chapter}」出1道填空题，用于{qa_type}。"
                    if qa_type == "课中随堂检测":
                        sp += "要求：考察关键概念理解，不要过于简单。输出格式：\n【题目】xxx\n【答案】xxx"
                    else:
                        sp += "输出格式：\n【题目】xxx\n【答案】xxx"
                else:
                    sp = f"你是遥感课程教师。请围绕「{qa_knowledge if qa_knowledge else qa_chapter}」出1道简答题，用于{qa_type}。"
                    if qa_type == "课中随堂检测":
                        sp += "要求：需要一定的思考和推导，非直接照搬课本。输出格式：\n【题目】xxx\n【答案要点】xxx"
                    else:
                        sp += "输出格式：\n【题目】xxx\n【答案要点】xxx"
                result = call_llm(sp, f"课本内容：\n{ctx}", max_tokens=500)
                if "【题目】" in result and ("【答案】" in result or "【答案要点】" in result):
                    q_part = result.split("【题目】")[1]
                    if "【答案】" in q_part:
                        q_text = q_part.split("【答案】")[0].strip()
                        a_text = q_part.split("【答案】")[1].strip()
                    elif "【答案要点】" in q_part:
                        q_text = q_part.split("【答案要点】")[0].strip()
                        a_text = q_part.split("【答案要点】")[1].strip()
                    else:
                        q_text = result
                        a_text = ""
                    if "【解析】" in a_text:
                        a_text = a_text.split("【解析】")[0].strip()
                else:
                    q_text = result
                    a_text = ""
                st.session_state["class_qa_generated_q"] = q_text
                st.session_state["class_qa_generated_a"] = a_text
                st.rerun()

        if "class_qa_generated_q" in st.session_state:
            qa_question = st.text_area("输入问题（可编辑）：", value=st.session_state["class_qa_generated_q"], key="class_qa_input_q_edit")
        if "class_qa_generated_a" in st.session_state:
            ref_answer = st.text_area("参考答案：", value=st.session_state["class_qa_generated_a"], key="class_qa_input_a_edit")
        else:
            ref_answer = st.text_area("参考答案（可选）：", key="class_qa_ref", placeholder="可留空，系统会自动评分")
        ...
        if st.button("📤 发布问题", key="class_qa_publish"):
            final_q = qa_question if qa_question else st.session_state.get("class_qa_generated_q", "")
            final_a = ref_answer if ref_answer else st.session_state.get("class_qa_generated_a", "")
            if final_q:
                class_qa.append({
                    "chapter": qa_chapter,
                    "type": qa_type,
                    "question": final_q,
                    "q_type": qa_question_type,
                    "reference": final_a,
                    "deadline": deadline,
                    "time": datetime.now().strftime("%Y-%m-%d %H:%M")
                })
                save_json(class_qa, CLASS_QA_FILE)
                st.success(f"✅ 已发布：{qa_type}（{qa_question_type}）- {final_q[:50]}...")
        ...
        if class_qa:
            ...
            if selected_qa_chapter == "全部":
                filtered_qa = class_qa
            else:
                filtered_qa = [item for item in class_qa if item.get('chapter') == selected_qa_chapter]
            ...
            for ch, types in chapter_dict.items():
                ...
                for tp, items in types.items():
                    ...
                    for idx, item in enumerate(items):
                        with st.expander(
                                f"{item.get('q_type', '')} | {item.get('time', '')} | {item['question'][:40]}..."):
                            ...
                            if st.button("🗑️ 删除", key=f"del_qa_{item.get('time', '')}_{idx}"):
                                class_qa.remove(item)
                                save_json(class_qa, CLASS_QA_FILE)
                                st.rerun()
    ...
    with tabs[2]:
        ...
        with col1:
            choice_count = st.number_input("选择题数量", min_value=0, max_value=10, value=2, key="quiz_choice")
        with col2:
            fill_count = st.number_input("填空题数量", min_value=0, max_value=10, value=1, key="quiz_fill")
        with col3:
            essay_count = st.number_input("简答题数量", min_value=0, max_value=10, value=1, key="quiz_essay")
        ...
        if total_count == 0:
            ...
            if st.button("🎲 生成章节检测", key="gen_quiz_btn"):
                ...
                with st.spinner(f"正在生成 {total_count} 道题目..."):
                    if choice_count > 0:
                        choice_questions = generate_quiz_questions(sel, "选择题", choice_count)
                        all_questions.append(("选择题", choice_questions))
                    if fill_count > 0:
                        fill_questions = generate_quiz_questions(sel, "填空题", fill_count)
                        all_questions.append(("填空题", fill_questions))
                    if essay_count > 0:
                        essay_questions = generate_quiz_questions(sel, "简答题", essay_count)
                        all_questions.append(("简答题", essay_questions))
                st.session_state["gen_quiz_mixed"] = all_questions
                st.rerun()

        if "gen_quiz_mixed" in st.session_state:
            st.markdown("---")
            st.markdown("### 生成的题目（可编辑）")
            all_questions = st.session_state["gen_quiz_mixed"]
            edited_all = []
                    ...
                    for block in text.split("题目")[1:]:
                        ...
                        if lines:
                            ...
                            for line in full_question.split("\n"):
                                if line.startswith("答案：") or line.startswith("正确选项内容：") or line.startswith(
                                        "解析：") or line.startswith("参考答案要点：") or line.startswith("参考答案"):
                                    break
                            ...
                            for line in lines:
                                if "答案：" in line:
                                    ans = line.split("答案：")[-1].strip()
                                elif "正确选项内容：" in line:
                                    ans = line.split("正确选项内容：")[-1].strip()
                                elif "参考答案要点：" in line:
                                    ans = line.split("参考答案要点：")[-1].strip()
                            if q_text:
                                questions.append(
                                    {"question": full_question, "answer": ans if ans else "教师未提供参考答案",
                                     "q_type": q_type})
                if questions:
                    quizzes.append({"chapter": sel, "q_type": "混合", "questions": questions})
                    save_json(quizzes, QUIZ_FILE)
                    st.success(f"✅ 发布 {len(questions)} 道题（选择{choice_count}/填空{fill_count}/简答{essay_count}）")
                    del st.session_state["gen_quiz_mixed"]
                    st.rerun()
                else:
                    st.error("题目解析失败，请检查格式")
        # 已发布的检测列表
        quizzes = load_json(QUIZ_FILE)
        if isinstance(quizzes, list) and quizzes:
            ...
            for idx, qz in enumerate(quizzes):
                q_type_info = qz.get("q_type", "混合")
                with st.expander(f"{qz['chapter']}（{q_type_info}，{len(qz['questions'])}题）"):
                    for i, qi in enumerate(qz['questions']):
                        st.write(f"**{i + 1}**（{qi.get('q_type', '')}）{qi['question']}")
                        st.write(f"答案：{qi.get('answer', '无')}")  # 加这行
                    if st.button(f"🗑️ 删除此检测", key=f"del_quiz_{idx}"):
                        quizzes.pop(idx)
                        save_json(quizzes, QUIZ_FILE)
                        st.success("🗑️ 已删除")
                        st.rerun()
    ...
    with tabs[3]:
        ...
        if paper_mode == "手动输入":
            ...
            if st.button("➕ 添加", key="add_paper_btn") and title:
                ...
                papers.append({"chapter": sel, "title": title, "author": author, "abstract": abstract})
                save_json(papers, PAPER_FILE)
                st.success("✅ 已添加")
                st.rerun()
        else:
            uploaded_papers = st.file_uploader("上传论文文件（可多选）", type=["pdf", "docx", "doc", "txt"],
                                               accept_multiple_files=True, key="paper_files_upload")
            if uploaded_papers:
                papers = load_json(PAPER_FILE)
                if not isinstance(papers, list): papers = []
                ...
                for up in uploaded_papers:
                    if up.name in existing_names:
                        continue
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    file_path = os.path.join(paper_dir, f"{ts}_{up.name}")
        ...
        if isinstance(papers, list) and papers:
            ...
            for idx, p in enumerate(papers):
                if p.get("chapter") != sel:
                    continue
                if p.get("is_file"):
                    with st.expander(f"📄 {p['title']}"):
                        st.write(f"**文件路径**：{p.get('file_path', '')}")
                        if os.path.exists(p.get('file_path', '')):
                            with open(p['file_path'], "rb") as f:
                                st.download_button("📥 下载论文", f.read(), file_name=p['title'], key=f"dl_paper_{idx}")
                        if st.button("🗑️ 删除", key=f"del_paper_{idx}"):
                            papers.pop(idx)
                            save_json(papers, PAPER_FILE)
                            st.rerun()
                else:
    ...
    with tabs[6]:
        ...
        with filter_col1:
            show_mode = st.selectbox("筛选：", ["全部", "仅显示错题", "仅显示未做"], key="overview_filter")
        ...
        with tab1:
            study_log = load_json(STUDY_LOG_FILE)
            students = study_log.get("students", {})
            all_kps = []
            ...
            if not all_kps:
                ...
                for sk, chs in students.items():
                    ...
                    sn = parts[1] if len(parts) > 1 else sk
                    learned = set(chs.get(view_ch, {}).get("learned", []))
                            ...
                            if os.path.exists(LOG_FILE):
                                ...
                                with open(LOG_FILE, "w", encoding="utf-8") as f:
                                    for line in all_lines:
                                        try:
                                            r = json.loads(line)
                                            key = f"{r.get('student_id', '')}_{r.get('student_name', '')}"
                                            if key != sk:
                                                f.write(line)
                                        except:
                                            f.write(line)
                            st.success(f"✅ {si}{sn}的数据已删除")
                            st.rerun()

                        st.progress(min(max(progress / total_kps, 0.0), 1.0) if total_kps > 0 else 0.0)

                        for kp_idx, kp in enumerate(all_kps):
                            kp_title = kp.get('title', f'知识点{kp_idx + 1}')
                            think_q = kp.get('think_question', '')

                            student_think = [r for r in analyze_logs_by_type("think")
                                             if r.get('chapter') == view_ch
                                             and f"{r.get('student_id', '')}_{r.get('student_name', '')}" == sk
                                             and r.get('kp_title') == kp_title]

                            if not think_q:
                                status_color = "#e0e0e0";
                                status_text = "—"
                            elif not student_think:
                                status_color = "#9e9e9e";
                                status_text = "未做"
                            else:
                            ...
                            if show_mode == "仅显示错题" and status_color not in ["#f44336", "#ff9800"]:
                                continue
                            if show_mode == "仅显示未做" and status_color != "#9e9e9e":
                                continue
                            ...
                            with col_status:
                                st.markdown(
                                    f"<div style='background:{status_color};color:white;text-align:center;border-radius:20px;padding:4px 12px;font-size:0.9em'>{status_text}</div>",
                                    unsafe_allow_html=True)

                            if think_q and student_think:
                                r = student_think[-1]
                                if st.button(f"查看详情", key=f"think_detail_{sk}_{kp_idx}"):
                                    st.session_state[f"think_detail_open_{sk}_{kp_idx}"] = not st.session_state.get(
                                        f"think_detail_open_{sk}_{kp_idx}", False)
                                if st.session_state.get(f"think_detail_open_{sk}_{kp_idx}", False):
                                    st.write(f"**思考题**：{think_q}")
                                    st.write(f"**学生回答**：{r.get('student_answer', '')}")
                                    st.caption(r.get('evaluation', ''))
                                    st.markdown("---")
        ...
        with tab2:
            ...
            if not cqr:
                ...
                pre = [r for r in cqr if r.get('type') == '课前预习检测']
                mid = [r for r in cqr if r.get('type') == '课中随堂检测']
                ...
                with qa_tab1:
                    if not pre:
                        ...
                        stu_dict = {}
                        for r in pre:
                            k = f"{r['student_id']}{r['student_name']}"
                            stu_dict.setdefault(k, []).append(r)
                        for stu, recs in stu_dict.items():
                            ...
                            with st.expander(f"{stu} | 共{len(recs)}题 | 正确{correct}"):
                                if st.button(f"🗑️ 删除{stu}的课前问答", key=f"del_cqa_pre_{stu}"):
                                    all_lines = []
                                    if os.path.exists(LOG_FILE):
                                        with open(LOG_FILE, "r", encoding="utf-8") as f:
                                            all_lines = f.readlines()
                                        with open(LOG_FILE, "w", encoding="utf-8") as f:
                                            for line in all_lines:
                                                try:
                                                    r = json.loads(line)
                                                    key = f"{r.get('student_id', '')}{r.get('student_name', '')}"
                                                    if not (key == stu and r.get('log_type') == 'class_qa' and r.get(
                                                            'type') == '课前预习检测'):
                                                        f.write(line)
                                                except:
                                                    f.write(line)
                                    st.success(f"✅ {stu}的课前问答已删除")
                                    st.rerun()
                                for r in recs:
                                    score = r.get('score', 0)
                        ...
                        with st.expander("📊 全班总览（课前预习）", expanded=True):
                            q_unique = {}
                            for r in pre:
                                q_text = r.get('question', '')
                                if q_text not in q_unique:
                                    q_unique[q_text] = {"question": q_text, "q_type": r.get('q_type', ''),
                                                        "answers": [],
                                                        "reference": r.get('reference', '') or r.get('evaluation', '')}
                                q_unique[q_text]["answers"].append(r)
                            st.write(f"**共 {len(q_unique)} 道预习题**")
                            for i, (q_text, q_data) in enumerate(q_unique.items()):
                                total = len(q_data["answers"])
                                correct = sum(1 for a in q_data["answers"] if a.get('score', 0) >= 80)
                                st.write(f"**第{i + 1}题**（{q_data['q_type']}）：{q_text[:200]}")
                                st.write(f"答对：{correct}人 | 答错：{total - correct}人")
                                st.write(f"**参考答案**：{q_data['reference'][:80] if q_data['reference'] else '无'}")
                                for a in q_data["answers"]:
                                    icon = "✅" if a.get('score', 0) >= 80 else "❌"
                                    st.write(
                                        f"{icon} {a.get('student_id', '')}{a.get('student_name', '')}：{a.get('student_answer', '')[:60]}")
                                st.markdown("---")
                ...
                with qa_tab2:
                    if not mid:
                        ...
                        for stu, recs in stu_dict.items():
                            correct = sum(1 for r in recs if r.get('score', 0) >= 80)
                            with st.expander(f"{stu} | 共{len(recs)}题 | 正确{correct}"):
                                if st.button(f"🗑️ 删除{stu}的课中问答", key=f"del_cqa_mid_{stu}"):
                                    all_lines = []
                                    if os.path.exists(LOG_FILE):
                                        with open(LOG_FILE, "r", encoding="utf-8") as f:
                                            all_lines = f.readlines()
                        ...
                        with st.expander("📊 全班总览（课中随堂）", expanded=True):
                            q_unique = {}
                            for r in mid:
                                q_text = r.get('question', '')
                                if q_text not in q_unique:
                                    q_unique[q_text] = {"question": q_text, "q_type": r.get('q_type', ''),
                                                        ...
                                                        "reference": r.get('reference', '') or r.get('evaluation', '')}
                                q_unique[q_text]["answers"].append(r)
                            st.write(f"**共 {len(q_unique)} 道随堂题**")
                            for i, (q_text, q_data) in enumerate(q_unique.items()):
                                total = len(q_data["answers"])
                                correct = sum(1 for a in q_data["answers"] if a.get('score', 0) >= 80)
                                st.write(f"**第{i + 1}题**（{q_data['q_type']}）：{q_text[:200]}")
            ...
            if not qr:
                                ...
                                with open(LOG_FILE, "w", encoding="utf-8") as f:
                                    for line in all_lines:
                                        try:
                                            r = json.loads(line)
                                            key = f"{r.get('student_id', '')}{r.get('student_name', '')}"
                                            if not (key == stu and r.get('log_type') == 'quiz'):
                                                f.write(line)
                                        except:
                                            f.write(line)
                            st.success(f"✅ {stu}的章节检测已删除")
                            st.rerun()
                        for i, r in enumerate(recs):
                            is_correct = r.get('score', 0) >= 80
                            icon = "✅" if is_correct else "❌"
                            q_type = r.get('q_type', '')
                            st.markdown(f"**{icon} 第{i + 1}题（{q_type}）**")
                            st.write(r['question'][:100])
                            st.write(f"**学生答案**：{r.get('student_answer', '')[:100]}")
                            if q_type != '选择题':
                                st.write(f"**参考答案**：{r.get('correct_answer', '')[:100]}")
                                    ...
                                    with open(LOG_FILE, "w", encoding="utf-8") as f:
                                        for line in all_lines:
                                            try:
                                                ...
                                                if record.get('timestamp') == r['timestamp']:
                ...
                with st.expander("📊 全班总览（章节检测）", expanded=True):
                    ...
                    for r in qr:
                        q_text = r.get('question', '')
                        if q_text not in q_unique:
                            q_unique[q_text] = {"question": q_text, "q_type": r.get('q_type', ''), "answers": [],
                    ...
                    for i, (q_text, q_data) in enumerate(q_unique.items()):
                        ...
                        st.write(f"**第{i + 1}题**（{q_data['q_type']}）：{q_text[:200]}")
                        st.write(f"答对：{correct}人 | 答错：{total - correct}人")
                        if q_data['q_type'] != '选择题':
                            st.write(f"**参考答案**：{q_data['correct_answer'][:80]}")
                        for a in q_data["answers"]:
                            icon = "✅" if a.get('score', 0) >= 80 else "❌"
                            st.write(
                                f"{icon} {a.get('student_id', '')}{a.get('student_name', '')}：{a.get('student_answer', '')[:60]}")
        ...
        if os.path.exists(LOG_FILE):
            with open(LOG_FILE, "r", encoding="utf-8") as f:
                st.download_button("📥 下载日志", f.read(), file_name="full_log.jsonl", key="dl_log")